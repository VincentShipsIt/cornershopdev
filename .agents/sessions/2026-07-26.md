...
## Session 6 — Stripe subscription lifecycle (#8)

### What was implemented

- Webhook-owned, idempotent account and subscription provisioning.
- Server-side validation of claim, site, email, price, plan and subscription
  metadata before granting ownership.
- Durable subscription lifecycle synchronization, stale-event protection,
  Customer Portal access and site-scoped paid-feature gates.
- Recoverable Checkout attempts with a short-lived HttpOnly return credential.
- Test/live Stripe setup, price rotation and rejected-event operations runbook.

### Production activation

Live Products, Prices, Portal settings, webhook registration and encrypted
Stripe parameters remain an explicit production operation after merge.
