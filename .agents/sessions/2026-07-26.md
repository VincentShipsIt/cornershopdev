...
**Duration:** ~2 hours
**Status:** In Progress (delivery gates pending)
...
subscription lifecycle ─► ACTIVE ─► publish/domain + monitoring entitlement
                       └─► INCOMPLETE / PAST_DUE / CANCELED ─► Customer Portal
```
...
- [x] Read issue #8 and all related open issues; confirmed no open PR overlap.
- [x] Used Fable (`CLAUDE_CONFIG_DIR=~/.claude-genfeedai`, `fable`, high) for a structured `PLAN_READY` implementation plan.
- [x] Required a valid, unexpired, unused hashed `ClaimInvitation` before Checkout and bound one idempotent Stripe session to it.
...
- [x] Gated domain publication/live custom-domain routing on an active configured subscription; exposed the same shared gate for future source monitoring.
...
- [x] Verification: 165 tests passed, 4 database-gated analytics tests skipped; typecheck passed; ESLint passed with one pre-existing generated warning; Prisma schema valid; production build passed; ShellCheck and host-launcher tests passed.
- [ ] Commit, push, ready PR, exact-head Opus PASS, and green PR CI.
...
- **Gate the live-domain publication boundary.** Safe draft/publish (#18) and recurring source monitoring (#14) are not implemented yet; their future mutations must call the shared billing gate.
...
- Commit and push the scoped branch, then open a ready PR for issue #8.
- Require green exact-head GitHub CI.
- Run Opus (`CLAUDE_CONFIG_DIR=~/.claude-genfeedai`, high) against the exact PR head; repair and re-verify if needed.
